...
export default function NavMenu() {
  ...
  return (
    <div className="relative w-screen">
        <div className={`bg-[#282526] hidden h-screen w-full fixed inset-0 lg:flex items-center justify-end
          ...
          <div className="w-2/6 flex flex-col items-center justify-center">
            <h1>Abid Hasan</h1>
            <p>01789507895</p>
            <p>mp.abidhasan@gmail.com</p>
          </div>
          <div className="w-2/6 h-96 flex items-center justify-center">
            <GlowingCube/>
          </div>
        </div>
        <div className={`bg-[#f56f5e] h-screen lg:w-2/6 w-screen fixed inset-0 flex items-center justify-center cursor-none
          ...
          <ul className="w-full flex flex-col space-y-4 font-extrabold cursor-none">
              ...
              <Link key={i} href={item.link} onClick={()=>setIsOpen(false)}>
                <li
                  ...
                  uppercase 
                  ${activePage === item.link ? 'text-red-500 font-extrabold' : 'font-extralight'}`}
                >
                    {item.title}
                </li>
              </Link>
